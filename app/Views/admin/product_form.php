...
<form
  action="<?= !empty($adminProductsEdit)
...
>
  ...
  <label class="block">
    ...
    <textarea
      ...
      rows="4"
      class="w-full border rounded p-2"
    ><?= htmlspecialchars($product['description'] ?? '', ENT_QUOTES) ?></textarea>
  </label>
  ...
  <label class="block">
    ...
    <?php if (!empty($product['image'])): ?>
      <img
        src="/assets/products/<?= htmlspecialchars($product['image'], ENT_QUOTES) ?>"
        alt="<?= htmlspecialchars($product['name'] ?? '', ENT_QUOTES) ?>"
        class="h-24 mt-2 object-cover rounded"
      >
    <?php endif; ?>
  </label>
